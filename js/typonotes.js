...
$(document).ready(function() {
	...
	$('.notes').click(function(event) {
		if($(event.target).hasClass('notes') && event.which === 1) {
			if( $('.notes').data('editing') == false && event.clientX == $('.notes').data('x') && event.clientY == $('.notes').data('y')) {
				...
				TNcreateNote({text:'',x:x,y:y}).show('scale',null,'fast',function() {
					$(this).trigger('click'); 
				}).TNinsertNote().find('.note-input').trigger('keydown');
			}
			else {
...
function TNloadNotes() {
	...
	$.ajax({
		...
		success: function(notes){
			...
			if(notes.length > 0) {
				var i = 0, limit = notes.length, busy = false; 
				var processor = setInterval(function() {
					if(!busy) {
						busy = true; 
				
						TNcreateNote(notes[i]).show('scale',null,'fast').find('.note-input').trigger('keydown');
	...
	$.ajax({
		...
		success: function(settings){
			...
			if(!$('#dot-footer').data('active')) 
				...
				$('.bar-wrapper').css({'bottom':'-125px'}).show();
				
			if(!$('#dot-grid').data('active'))
				$('.grid').show();
		},
	});
}
...
function TNcreateNote(data) {
	...
	// Create new Note
	var $t = $('.note-template').clone().removeClass('note-template').appendTo('.notes');
	...
	.resizable({ aspectRatio: 1/1, handles: 'se', minHeight: 32, minWidth: 32, maxHeight: 256, maxWidth: 256 } )
	...
	.bind("dragstop", function(event, ui) {
		var $note = $(this);
		$('.pins').fadeOut('fast');
		if($(event.originalEvent.target).hasClass('dot') || $(event.originalEvent.target).parents('.dot').length) {
			
			// Delte Dot
			if($(event.originalEvent.target).hasClass('delete-all') || $(event.originalEvent.target).parents('.dot').hasClass('delete-all')) {
				$(this).find('.delete').click();
			}
			
			// Goolge Tasks Pin
			if($(event.originalEvent.target).hasClass('pin-tasks') || $(event.originalEvent.target).parents('.dot').hasClass('pin-tasks')) {	
				var data = {
					wall:$('.notes').data('wall'),
					noteID:$note.attr('id'),
				...
				if(typeof $(event.originalEvent.target).attr('id') != 'undefined')
					data.taskListID = $(event.originalEvent.target).attr('id').split('-')[2];
				else if(typeof $(event.originalEvent.target).parents('.dot').attr('id') != 'undefined')
					data.taskListID = $(event.originalEvent.target).parents('.dot').attr('id').split('-')[2];
					...
					success: function(data) {
						if(typeof data.error != 'undefined') {
							if(data.error == 'Forbidden') {
								$('#'+data.note.id).TNresetNote(data.note).find('.note-input').trigger('keydown');
							}
							setError(data.error);
						}
						else {
				...
				$note.data('color',6).TNchangeColor({color:NoteColors[6]}).find('#btn-complete').show().siblings('#btn-delete').hide();
			}
	...
	.click(function(event) {
		if( $(event.target).hasClass('note-text') || $(event.target).hasClass('note-wrapper')) {
			$(this).find('.note-header').show();
			$(this).find('.note-text').hide().siblings('.note-input').show().select();
			$('.notes').data('editing',true);
		}
	})
	// Note Input
	.find('.note-input').val(data.text)
	...
	.blur(function(event) {
		$(this).parents('.note-wrapper').TNhideInput().TNupdateNote().find('.note-header').hide();
	})
	// Set passed Text
	.siblings('.note-text').html(replaceURLWithHTMLLinks(data.text));
	
	// Header Menu
	$t.find('#btn-delete').click(function() {
		var api = $(this).data("tooltip");
		var $tip = $(api.getTip());
		$tip.remove();
		$t.TNsetDeleted().TNremoveNote();
	})
	.siblings('#btn-color').click(function(event) {
	...
	.siblings('#btn-font').click(function() {
		var font = $t.TNgetNextFont();
		$t.TNchangeFont({font:font});
	})
	.siblings('#btn-complete').click(function() {
	...
	// Set passed values
	$t.TNresizeNote({scale:data.scale}).TNchangeColor({color:NoteColors[data.color]}).TNchangeFont({font:NoteFonts[data.font].font});
	return $t;
}
...
$.fn.TNdisplayCompleted = function() {
	...
	if($note.data('taskCompleted')) {
		$note.find('.note-text, .note-input').css({'text-decoration': 'line-through'});
...
$.fn.TNinitNoteHover = function() {
	...
	return this.each(function() {
		$(this).hover(function() {
			...
			$(this).find('.note-header').stop().fadeTo('fast',0, function() {$(this).hide();});
		});
    });
};
...
function setError(error) {
	if(error)
		$('.error').html(error).addClass('red');
	else
		$('.error').html('No errors').removeClass('red');
}
...
$.fn.TNsetDeleted = function() {
	...
	$.ajax({
		...
		success: function(data) {
			if(typeof data.error != 'undefined') {
				if(data.error == 'Forbidden') {
					TNcreateNote(data.note).show('scale',null,'fast').find('.note-input').trigger('keydown');
				}
				setError(data.error);
			}
			else {
...
$.fn.TNchangeColor = function() {
	...
	$t.css('background-color','#'+color).find('.note-button').css({'background-color':'#'+color /*, opacity:0.75*/});
...
$.fn.TNchangeLineheight = function() {
	...
	$t.find('.note-input')
	.css({'font-size':(fontsize*scale)+'px', 'line-height':(lineheight*scale)+'px'})
	.parents('.note').css({'font-size': (fontsize*scale)+'px','line-height':(lineheight*scale)+'px'})
...
$.fn.TNresizeNote = function() {
	...
	$t.animate({'width':(200*scale), 'height':(200*scale)},{queue:false, duration:duration})
	.find('.note').animate({'font-size': ($t.data('font-size')*scale) + 'px', 'line-height': ($t.data('line-height')*scale) + 'px'},{queue:false, duration:duration})
	.find('.note-input').animate({'font-size': ($t.data('font-size')*scale) + 'px', 'line-height': ($t.data('line-height')*scale) + 'px', 'width': (180*scale) + 'px', 'height': (180*scale) + 'px'},{queue:false, duration:duration});
...
function TNinitMouseMove() {
	...
	.keydown(function(event) {
		if(event.keyCode == '16')
			$('.notes').data('shift',true);
	})
	.keyup(function(event) {
		if(event.keyCode == '16')
			$('.notes').data('shift',false);
	})
	...
	.mousedown(function (event) {
		if($(event.target).hasClass('notes') !== true)
			return;
		var p = $(this).position();
		$(this)
		.data('down', true)
	...
	.mousemove(function (event) {
		if($(this).data('down')) {
      console.log('test');
			var dL = $(this).data('pLeft') + ($(this).data('x') - event.clientX)*1.5,
...
function TNinitFooter() {
	...
	$('.header-wrapper').data('closed',false).click(function(event) {
		if($(event.target).hasClass('header-wrapper')) {
			if($(this).data('closed')) 
				$(this).animate({'top':'0px'},'fast');
			else
				$(this).animate({'top':'-22px'},'fast');
...
function TNinitBackground() {
	...
	var $temp = $('.quadrant');
	for(i=1; i<180; i++) {
		$temp.clone().appendTo('.grid')
		.find('.x-length').html(480*(i%9)+'px')
		.siblings('.y-length').html(480*Math.floor(i/9)+'px');
	}
}
...
$.fn.TNupdateSettings = function() {
	...
	$.ajax({
		...
		success: function(data) {
			if('value' in data)
				$t.TNtoggleDot({active:data.value})
		}
	});
	return $t;
};
...
function TNinitUI() {
	...
	$('#dot-footer').click(function() {
		...
		$(this).data('active',!value).TNupdateSettings();
		if(value) 
			$('.bar-wrapper').animate({'bottom':'0px'},'normal');
		else
			$('.bar-wrapper').animate({'bottom':'-125px'},'normal');
	});
...
function TNinitChannel() {
	...
	if( typeof $('.notes').data('token') != 'undefined') {
		...
		socket.onmessage = function(message) {
			...
			if(data.wall_id == $('.notes').data('wall_id') && data.visitor_id != $('.notes').data('visitor_id')) {
				var $t = $('#'+note.id);
...
$.fn.TNresetNote = function() {
	...
	if(typeof data.y == 'undefined')
		data.y = $note.data('y');;
	if(typeof data.scale == 'undefined')
		data.scale = $note.data('scale');
	if(typeof data.color == 'undefined')
		data.color = $note.data('color');
	if(typeof data.font == 'undefined')
		data.font = $note.data('font');
	if(typeof data.text == 'undefined')
	...
	if(typeof data.z == 'undefined')
		data.z = $note.data('z');
	if(typeof data.counter == 'undefined')
		data.counter = $note.data('counter');
	if(typeof data.taskCompleted == 'undefined')
